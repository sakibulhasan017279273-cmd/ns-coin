...
LANG = {
    "bn": {
        ...
        "order_submitted"     : (
            ...
            "{{icon}} পেমেন্ট পাবেন:  <b>{{method}}</b> — <code>{{masked_num}}</code>\n"
        ...
        "admin_new_order"     : (
            ...
            "📊 রেট:      <b>{{rate}}৳</b> / ১০০০\n"
            "💵 মোট:     <b>{{taka}}৳</b>\n"
            "{{icon}} {{method}}: <code>{{full_num}}</code>\n"
            "🆔 অর্ডার:  <b>#{{order_id}}</b>\n"
            "🕐 সময়:    {{time}}\n"
            f"{LINE}\n"
    ...
    "en": {
        ...
        "order_submitted"     : (
            ...
            "{{icon}} You'll receive payment via:  <b>{{method}}</b> — <code>{{masked_num}}</code>\n"
        ...
        "unknown_command"     : (
            ...
            "💎 To sell coins, tap the <b>\"💎 Sell Coins\"</b> button."
        ),
...
def get_usd_rate() -> float:
    """১ ডলার = কত টাকা (BEP-20 payout-এর USD হিসাব দেখাতে ব্যবহৃত)।"""
    return float(get_setting("usd_rate", "122"))

def get_bep20_min_usd() -> float:
    """BEP-20-তে payout নিতে হলে সর্বনিম্ন কত ডলারের সমতুল্য বিক্রি করতে হবে।"""
    return float(get_setting("bep20_min_usd", "0.50"))
...
def get_stats() -> dict:
    ...
    return {
        ...
        "revenue": int(revenue), "coins_sold": int(coins_sold),
...
def t_bn(key: str, **kwargs) -> str:
    """যেসব মেসেজ চ্যানেলে যায়, সেগুলো user-এর ভাষা নির্বিশেষে সবসময় বাংলায় দেখানোর জন্য।"""
    text = LANG["bn"].get(key, key)
    for k, v in kwargs.items():
        text = text.replace("{{" + k + "}}", str(v))
    return text
...
def get_required_channels() -> list:
    ...
    if raw is None:
        # প্রথমবার — CONFIG-এ যা হার্ডকোড করা ছিল সেটাই settings-এ সেভ করে নিন
...
async def check_channel_member(bot, user_id: int):
    """Returns True only if the user has joined ALL required channels (admin-configurable).
    ...
    for channel in channels:
        try:
            ...
            logging.info(f"[join-check] BadRequest uid={user_id} channel={channel}: {e}")
            return False
        except telegram.error.Forbidden as e:
            logging.warning(f"[join-check] Forbidden — বট সম্ভবত '{channel}'-এ Admin না! uid={user_id}: {e}")
            return None
        except Exception as e:
            logging.warning(f"[join-check] error uid={user_id} channel={channel}: {e}")
            return None
    return True
...
ADMIN_RATE_INPUT, ADMIN_MSG_INPUT, ADMIN_ANNOUNCE_INPUT, ADMIN_BROADCAST_INPUT, ADMIN_SETTING_INPUT, ADMIN_RULES_INPUT, ADMIN_USERNAME_INPUT, ADMIN_CHANNEL_INPUT = range(100, 108)
...
async def method_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    icon = method_icon(method)
    if method == "BEP-20":
        taka = context.user_data.get("taka", 0)
        usd_rate = get_usd_rate()
        usd_amount = round(taka / usd_rate, 2) if usd_rate > 0 else 0
        min_usd = get_bep20_min_usd()
        if usd_amount < min_usd:
            await query.answer(
                f"⚠️ BEP-20-এ payout নিতে কমপক্ষে ${min_usd} এর সমতুল্য সেল করতে হবে (এখন ${usd_amount})।",
                show_alert=True
            )
            return METHOD
        await query.answer()
        ...
        await query.edit_message_text(
            t(uid, "ask_bep20", usd=f"{usd_amount:.2f}"), parse_mode="HTML"
        )
        return NUMBER
    await query.answer()
    context.user_data["method"] = method
...
async def wallet_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "usesaved":
        ...
        if method == "BEP-20":
            ...
            await query.edit_message_text(t(uid, "ask_bep20", usd=f"{usd_amount:.2f}"), parse_mode="HTML")
        else:
            await query.edit_message_text(t(uid, "method_new_ask", icon=icon, method=method), parse_mode="HTML")
        return NUMBER
...
async def get_proof(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    caption = t(uid, "admin_new_order",
                ...
                icon=icon, method=d['method'], full_num=d['number'],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action in ("paid", "reject"):
        ...
        if action == "paid":
            ...
            channel_msg = t_bn("reject_channel",
                            name=display_name, qty=qty, taka=taka,
                            icon=icon, method=method, masked_num=mask_number(number),
                            order_id=order_id, time=now_str())
            await query.edit_message_caption(
...
def admin_reply_keyboard():
    return ReplyKeyboardMarkup(
        [
            ...
            [KeyboardButton(ABTN_CHANNELS),     KeyboardButton(ABTN_MENU_TOGGLE)],
            [KeyboardButton(ABTN_VIEW)],
...
async def admin_menu_ref(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"💵 USD Rate:      <b>১$ = {usd_rate}৳</b>\n"
        f"🪙 BEP-20 Min Sell: <b>${bep20_min}</b>\n"
        f"{LINE}\n"
...
async def admin_ref_settings_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        for line in text.splitlines():
            ...
            elif key in ("usdrate", "dollarrate", "rate_usd"):
                set_setting("usd_rate", str(val_str))
                changed.append(f"✅ USD Rate: ১$ = {val_str}৳")
            elif key in ("bep20min", "minusd", "bepmin"):
                set_setting("bep20_min_usd", str(val_str))
                changed.append(f"✅ BEP-20 Min Sell: ${val_str}")
    except Exception:
        await update.message.reply_text(
...
async def admin_menu_channels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channels:
        ...
        listing = "  (কোনো চ্যানেল সেট নেই — বট এখন কাউকে জয়েন করতে বলবে না)"
    await update.message.reply_text(
        f"📢 <b>Join Channel সেটিংস</b>\n{LINE}\n"
        f"বর্তমান চ্যানেল লিস্ট:\n{listing}\n{LINE}\n\n"
        f"➕ নতুন চ্যানেল যোগ করতে: <code>+@channelusername</code>\n"
        f"➖ চ্যানেল বাদ দিতে: <code>-@channelusername</code>\n\n"
        f"একসাথে একাধিক লাইনও পাঠাতে পারেন, যেমন:\n"
        f"<code>+@BDincomeTV\n+@nscoinpaymentchannel\n-@oldchannel</code>\n\n"
        f"💡 চ্যানেল থেকে বাদ দিতে চাইলে সব চ্যানেল remove করে দিন (join gate পুরোপুরি বন্ধ হয়ে যাবে)।\n"
        f"বাতিল করতে /cancel লিখুন।",
        parse_mode="HTML"
    )
...
async def admin_channel_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for line in text.splitlines():
        ...
        if line.startswith("+"):
            ch = line[1:].strip()
            if re.match(r'^@?[A-Za-z0-9_]{4,32}$', ch):
                if add_required_channel(ch):
                    added.append(ch if ch.startswith("@") else f"@{ch}")
            else:
                invalid.append(line)
        elif line.startswith("-"):
            ch = line[1:].strip()
            if re.match(r'^@?[A-Za-z0-9_]{4,32}$', ch):
                if remove_required_channel(ch):
                    removed.append(ch if ch.startswith("@") else f"@{ch}")
            else:
                invalid.append(line)
        else:
            invalid.append(line)
    reply_lines = []
    if added:
        reply_lines.append("✅ যোগ হয়েছে: " + ", ".join(added))
    if removed:
        reply_lines.append("🗑️ বাদ দেওয়া হয়েছে: " + ", ".join(removed))
    if invalid:
        reply_lines.append("⚠️ বোঝা যায়নি: " + ", ".join(invalid))
    if not reply_lines:
        reply_lines.append("⚠️ কোনো পরিবর্তন হয়নি। প্রতিটা লাইন অবশ্যই + বা - দিয়ে শুরু করতে হবে।")
    await update.message.reply_text("\n".join(reply_lines))
    channels = get_required_channels()
    listing = "\n".join([f"  {i+1}. {ch}" for i, ch in enumerate(channels)]) or "  (কোনো চ্যানেল নেই)"
    await update.message.reply_text(f"📢 বর্তমান চ্যানেল লিস্ট:\n{listing}")
    await update.message.reply_text("🛠️ Admin Panel:", reply_markup=admin_reply_keyboard())
    return ConversationHandler.END
...
async def admin_menu_view(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sold_today = get_today_sold_qty()
    channels = get_required_channels()
    channels_txt = "\n".join([f"  • {ch}" for ch in channels]) if channels else "  (কোনো চ্যানেল সেট নেই)"
    receive_val = get_receive_username()
    receive_display = receive_val if _BEP20_RE.match(receive_val) else f"@{receive_val}"
    msg = (
        f"ℹ️ <b>বর্তমান সেটিংস</b>\n{LINE}\n"
        f"💹 রেট:\n{tiers_txt}\n{LINE}\n"
        f"🛠️ Maintenance: <b>{maint}</b>\n{LINE}\n"
        ...
        f"💳 Receive Username/Address: <b>{receive_display}</b>\n{LINE}\n"
        f"📢 Join Channels:\n{channels_txt}\n{LINE}\n"
        f"📣 Announcement:\n{ann}"
    )
    await update.message.reply_text(msg, parse_mode="HTML")
...
async def menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # কোনো বাটন/কমান্ডের সাথেই মিলল না — ইউজারকে বিভ্রান্ত না করে সরাসরি বুঝিয়ে দিন
