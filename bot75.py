...
def build_main_menu(user_id: int) -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        [
            [KeyboardButton(L["btn_order"])],
            [KeyboardButton(L["btn_price"]),    KeyboardButton(L["btn_wallet"])],
            [KeyboardButton(L["btn_history"]),  KeyboardButton(L["btn_support"])],
            [KeyboardButton(L["btn_referral"]), KeyboardButton(L["btn_balance"])],
            [KeyboardButton(L["btn_rules"]),    KeyboardButton(L["btn_leaderboard"])],
            [KeyboardButton(L["btn_lang"])],
        ],
...
async def admin_ref_settings_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    min_w = get_min_withdraw()
    await query.edit_message_text(
        f"⚙️ <b>Referral & Withdraw Settings</b>\n{LINE}\n"
        f"🔗 বর্তমান Referral Bonus: <b>{int(bonus) if bonus==int(bonus) else bonus}৳</b>\n"
        f"💸 বর্তমান Min Withdraw:  <b>{int(min_w) if min_w==int(min_w) else min_w}৳</b>\n\n"
        f"{LINE}\n"
        f"নতুন সেটিং পাঠান এই ফরম্যাটে:\n"
        f"<code>bonus-10\nminwithdraw-50</code>\n\n"
        f"(শুধু একটি পরিবর্তন করতে চাইলে শুধু সেই লাইনটি পাঠান।)\n"
        f"বাতিল করতে /cancel লিখুন।",
        parse_mode="HTML"
    )
...
async def admin_ref_settings_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        for line in text.splitlines():
            ...
            if line.startswith("bonus-"):
                ...
                changed.append(f"✅ Referral Bonus: {val}৳")
            elif line.startswith("minwithdraw-"):
                val = float(line.split("-", 1)[1])
                set_setting("min_withdraw", str(val))
                changed.append(f"✅ Min Withdraw: {val}৳")
    except Exception:
        await update.message.reply_text(
            "⚠️ ফরম্যাট ভুল।\n<code>bonus-10\nminwithdraw-50</code>",
            parse_mode="HTML"
    ...
    if changed:
        ...
        await update.message.reply_text("⚠️ কোনো পরিবর্তন হয়নি।")
    await update.message.reply_text("🛠️ Admin Panel:", reply_markup=admin_reply_keyboard())
    return ConversationHandler.END
...
async def admin_menu_receive_un(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"💳 বর্তমান Receive Username: <code>@{current}</code>\n\n"
        f"নতুন Telegram username পাঠান (@ ছাড়া, যেমন <code>sakib173087</code>)।\n"
        f"বাতিল করতে /cancel লিখুন।",
        parse_mode="HTML"
    )
    return ADMIN_USERNAME_INPUT
...
async def admin_username_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    label = "Support Username" if key == "support_username" else "Receive Username"
    new_username = update.message.text.strip().lstrip("@")
    if not re.match(r'^[A-Za-z0-9_]{4,32}$', new_username):
        await update.message.reply_text(
            "⚠️ ভুল ফরম্যাট। শুধু অক্ষর, সংখ্যা, এবং _ ব্যবহার করে username দিন (@ ছাড়া)।"
        )
        return ADMIN_USERNAME_INPUT
    set_setting(key, new_username)
    await update.message.reply_text(f"✅ {label} আপডেট হয়েছে: <code>@{new_username}</code>", parse_mode="HTML")
    await update.message.reply_text("🛠️ Admin Panel:", reply_markup=admin_reply_keyboard())
    return ConversationHandler.END
